document.addEventListener('DOMContentLoaded', function() {
    ...
    projectCards.forEach(card => {
        ...
        links.forEach((link, index) => {
            link.style.transform = `translateY(${10 * (index + 1)}px)`;
            link.style.opacity = '0';
            link.style.transition = `transform 0.3s ${index * 0.1}s, opacity 0.3s ${index * 0.1}s`;
        });
    });
...
darkModeToggle.addEventListener('click', () => {
    ...
    if (document.body.classList.contains('light-mode')) {
        ...
        document.documentElement.style.setProperty('--secondary-light', '#e9ecef');
    } else {
        darkModeToggle.innerHTML = '<i class="fas fa-moon"></i>';
        ...
        document.documentElement.style.setProperty('--secondary-light', '#112240');
    }
});
});
